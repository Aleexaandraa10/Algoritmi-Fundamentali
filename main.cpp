...
/*
//------------------------------------EXERCITIU 1 (Prim, cel putin 2 APM)------------------------
//Fie G un graf neorientat cu n noduri, numerotate de la 1 la n, si m muchii.
// Sa se determine daca G are cel putin doi arbori partiali distincti
// (= care difera prin cel putin o muchie). In caz afirmativ, sa se afiseze doi arbori partiali distincti ai lui G.

//------------------------------------EXERCIIU 2 (Sortare Topologica, lg max cu k destinatii)-----------------------------------------
// Fie G orientat aciclic, n noduri, m arce. Fie un sir s de valori nat de lg k numit sir de destinatii
// Sa se det. un drum de lg maxima in G care se termina in una dintre cele k destinatii

//-----------------------------------EXERCITIU 3 (flux, NU se pot vaccina)-----------------------------------------
//Tara AF a fost lovita de o pandemie numita RESTANTA. Astfel, cercetatorii au descoperit un vaccin
// care sa rezolve aceasta problema si l-au distribuit in  centre de vaccinare (numerotate de la  la ).
// Cele  persoane din tara (numerotate de la  la ) vor sa se vaccineze, doar ca acestea nu pot
// merge la orice centru, ci fiecare are o lista de centre la care poate ajunge. Totodata, pentru fiecare
// dintre cele  centre se cunoaste numarul de doze de vaccin (adica numarul maxim de persoane
// care se pot vaccina la acel centru). Sa se determine numarul minim de persoane care NU se pot vaccina,
...
//     Funcție pentru determinarea Arborelui Parțial Minim (APM) folosind algoritmul Prim
    ...
    void drumMaximSpreDestinatie() {
        ...
        //daca fac asa merg input 1 si 3 nu 2//lungimeMaxima[topSort[0]] = 0;  // pornim de la nodul 1


        for (int nod : topSort) {
            if (lungimeMaxima[nod] == INT_MIN) {lungimeMaxima[nod] = 0;}// aici e modificarea!!!!!, tb sa lasam pos. de plecare din orice nod
...
// ---------------------------EXERCITIU 1(ascendenti si descendenti in arbore)-----------------------
// arbore cu n noduri si radacina r, se da un nod c
//sa se det ascendentii si descendentii nodului c

//----------------------------EXERCITIU 2(Kruskal, verif poz muchie)----------------------------
//Graf conex neor cu n noduri si m muchii, se dau k valori
// Pt fiecare din cele k valori sa se verifice daca muchia cu indicele respectiv
//se afla in cel putin un APM. Indicele unei muchii reprezinta pozitia ei(de la 1 incepe)
//in lista de muchii din datele de intrare

//-------------------------------EXERCITIU 3(flux max, marim fluxul cu un p)----------------------------------
//O retea de transport N cu sursa 1 si destinatia n
//a)Sa se afiseze val max a unui Flux in N
//b) Se da un nr p. Se creste cu p capacitatea unui arc care iese din sursa
...
void cautaAscendenti(int radacina, int target, unordered_map<int, vector<int>>& tree, set<int>& visisted) {
    ...
    while(!q.empty()) {
        ...
        for(int vecin : tree[aux]) {
            //Se verifică dacă nodul vecin nu are deja un părinte în dicționarul tati.
            // daca acel find returneaza tati.end() inseamna ca vecin nu a fost gasit
...
int main() {
    ...
    vector<tuple<int, int, int>> muchiiDinSursa;
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (x == 1) {
            muchiiDinSursa.emplace_back(x, y, c);
        }
    }

    // Calculează fluxul maxim inițial
    int flux_initial = edmondsKarp(1, n, n, graf);
    //cout << "Flux maxim initial: "
    cout << flux_initial << "\n";

    // Testăm creșterea capacității fiecărei muchii din sursă
    int max_diferenta = 0;
    for (auto &[u, v, c] : muchiiDinSursa) {
    ...
    //cout << "Cresterea maxima a fluxului: "
    cout << max_diferenta << "\n";
...
//--------------------------------------EXERCITIU 1(dijkstra)---------------------------
// G orientat ponderat cu n noduri si m muchii
// Sa se det nodurile cu distanta cea mai mare fata de nodul 1

//--------------------------------------EXERCITIU 2 (2 grafuri doar cu muchii de 0 si de 1) -------------------
//G neor ponderat cu n noduri si m muchii. Costul fiecarei muchii este 0 sau 1
//Sa se raspunda la Q intrebari de forma "exista un drum de la x la y mergand doar pe
//muchii de cost c?"

//--------------------------------------EXERCITIU 3(flux cu capacitate 1)------------------------------------
//Harta unui joc este formata din n camere si , portale. La inceputul fiecarei zile te afli in camera
// 1 si vrei sa ajungi in camera n (dupa ce ajungi in camera n se termina ziua si te intorci in camera 1).
// Un portal poate fi folosit cel mult o data de-a lungul intregului joc.
// Sa se determine numarul maxim de zile care poate fi jucat acest joc?
...
//using namespace std;
//
//vector<int> dijkstra(int start, int n, vector<vector<pair<int, int>>> graf) {
//
...
//        if (distCurenta > dist[nodCurent]) continue;
//
//        for (auto [vecin, greutate] : graf[nodCurent]) {
//            if (dist[nodCurent] + greutate < dist[vecin]) {
//                dist[vecin] = dist[nodCurent] + greutate;
//                minHeap.push({dist[vecin], vecin});
//            }
...
//
//
//    vector<int> dist = dijkstra(1, n, graf);
//
//
//    for (int i = 2; i <= n; ++i) {
//        if (dist[i]>max){
...
int main(){
    int n, m, u, v;
    cin >> n >> m;
    vector<tuple<int, int, int>> graf(n+1);
    for(int i=1; i<=m; i++){
        cin >> u >> v ;
        graf.emplace_back(u, v, 1);
    }
    cout << edmondsKarp(1,n,n,graf);
    return 0;
}
 */
...
*/
                                        ...
                                        // PUNCT DE PORNIRE IN FIECARE PROBLEMA UNDE CITESC SI AFISEZ GRAFUL
//#include <iostream>
//#include <fstream>
//#include <vector>
//#include <tuple> // pentru std::tuple
